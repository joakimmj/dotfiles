...
return {
  {
    ...
    keys = {
      ...
      },
      {
        "<leader>/",
        function()
          Snacks.picker.grep({ glob = vim.fn.expand("%:t"), need_search = false })
        end,
...
transparent = false
function M.toggle_transparancy()
  transparent = not transparent
  if transparent then
    vim.api.nvim_set_hl(0, "Normal", { fg = redox.fg, bg = "none" })
    vim.api.nvim_set_hl(0, "NormalFloat", { fg = redox.fg, bg = "none" })
    vim.api.nvim_set_hl(0, "FloatBorder", { fg = redox.overlay, bg = "none" })
    vim.api.nvim_set_hl(0, "SignColumn", { fg = redox.muted, bg = "none" })
  else
...
function M.setup(opts)
  opts = opts or { transparent = false }
  vim.opt.termguicolors = true
  vim.cmd("hi clear")
  ...
  transparent = not opts.transparent
  M.toggle_transparancy()
